...
> Preload Critical Images in Classic Monks adds preload hints for above-the-fold images. These images load immediately while other images are lazy loaded.
...
Lazy loading all images (including above-the-fold) can cause a flash of unloaded content. Preloading critical images ensures the visible content loads immediately.
...
Hero images should load immediately (not lazily) to avoid a flash of empty space.

### Above-the-fold images

Any image visible on first load should be preloaded, not lazy loaded.
...
This feature registers 1 WordPress hook in `monks-preload.php`:

**Actions:**

- `wp_head` calls `cm_preload_critical_images()` (Outputs link preload tags for featured images and logos (priority 1))
