...
> Disable Lazy Loading for Admin Users in Classic Monks disables lazy loading for logged-in administrators, ensuring they see all content immediately.
...
Lazy loading can interfere with admin workflows (e.g., image editing, content review). Disabling it for admins ensures they see the full page immediately.
...
When editing content, admins need to see all images immediately, not wait for lazy loading.

### Debugging

When debugging lazy loading issues, disabling it for admins helps isolate the problem.
