...
> Lazy Load Background Images in Classic Monks defers CSS background image loading until the element enters the viewport.
...
CSS background images are not detected by standard lazy loading. This feature adds support for lazy loading background images, reducing initial page load time.
...
Hero sections with background images benefit from lazy loading. The image loads only when the user scrolls to it.

### Parallax sections

Parallax sections with background images load faster with lazy loading.
