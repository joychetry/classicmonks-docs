...
> Unload Images in Classic Monks prevents specific images from loading on pages where they are not needed. Reduces image load on non-relevant pages.
...
Some images are loaded on every page via headers, footers, or sidebars. Unloading them on pages where they're not visible reduces page weight.
...
Blog posts or text pages that don't need header images can unload them.

### Mobile optimization

On mobile, unloading non-essential images improves load time.
