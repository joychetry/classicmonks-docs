...
> Lazy Load HTML5 Videos in Classic Monks defers video element loading until the video enters the viewport.
...
Video elements are heavy resources. Lazy loading them reduces initial page load time by loading only the visible videos.
...
Pages with multiple video elements benefit from lazy loading. Only the visible videos load initially.

### Background videos

Background video elements load lazily, improving initial page speed.
