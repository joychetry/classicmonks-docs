---
title: "How to Disable RSS Feeds in WordPress"
...
> Disable RSS Feeds in Classic Monks prevents WordPress from generating RSS and Atom feeds, reducing server load and unnecessary content exposure.
...
Disable RSS Feeds in Classic Monks prevents WordPress from generating RSS and Atom feeds, reducing server load and unnecessary content exposure. This is one of many WordPress optimization features available in the Performance tab.

## Why You Need It

WordPress generates RSS feeds for posts, comments, and categories by default. For sites that don't offer RSS, this is unnecessary overhead.
...
If your site doesn't offer RSS feeds (e.g., most e-commerce sites), the feed generation is unnecessary overhead.

### Privacy

RSS feeds expose full post content to any visitor. Disabling them reduces content exposure.
...
This feature registers 5 WordPress hooks in `disable-rss-feeds.php`:

**Actions:**

- `do_feed` calls `cm_disable_rss_feeds()` (Intercepts RSS feed requests (priority 1))
- `do_feed_rdf` calls `cm_disable_rss_feeds()` (Intercepts RDF feed requests)
- `do_feed_rss` calls `cm_disable_rss_feeds()` (Intercepts RSS 1.0 feed requests)
- `do_feed_rss2` calls `cm_disable_rss_feeds()` (Intercepts RSS 2.0 feed requests)
- `do_feed_atom` calls `cm_disable_rss_feeds()` (Intercepts Atom feed requests)

```php
// Hooked in disable-rss-feeds.php
add_action( 'do_feed', 'cm_disable_rss_feeds' );
```
