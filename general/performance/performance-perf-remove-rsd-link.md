---
title: "How to Remove RSD Link in WordPress"
...
> Remove RSD Link in Classic Monks strips the RSD (Really Simple Discovery) link tag from the page head, reducing unnecessary HTTP headers and page weight.
...
Remove RSD Link in Classic Monks strips the RSD (Really Simple Discovery) link tag from the page head, reducing unnecessary HTTP headers and page weight. This is one of many WordPress optimization features available in the Performance tab.

## Why You Need It

The RSD link is a legacy feature for blog clients. Modern WordPress doesn't need it, but it still loads by default.
...
Fewer link tags in the head means cleaner HTML. Search engines prefer lean, focused HTML.

### Privacy

The RSD link reveals the XML-RPC endpoint. Removing it reduces the information exposed to attackers.
...
- `pre_cm_update_option_remove_rsd_link` calls `cm_toggle_rsd_link()` (Adds/removes wp_head rsd_link action)

```php
// Hooked in head-cleanup.php
add_filter( 'pre_cm_update_option_remove_rsd_link', 'cm_toggle_rsd_link' );
```
