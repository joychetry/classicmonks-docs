...
> Customize the WordPress admin toolbar in Classic Monks. Reorder, hide, or reorganize toolbar items.
...
The default WordPress admin toolbar may have items you don't use. Customizing it creates a cleaner, more focused toolbar.
...
This feature registers 3 WordPress hooks in `menu-manager/class-toolbar-manager.php`:

**Actions:**

- `wp_before_admin_bar_render` calls `CM_Toolbar_Manager::capture_and_modify_toolbar()` (Captures and modifies admin toolbar nodes (priority 999999))
- `wp_ajax_cm_save_toolbar_menu` calls `CM_Toolbar_Manager::ajax_save_toolbar_settings()` (AJAX handler for saving toolbar settings)
- `wp_ajax_cm_reset_toolbar_menu` calls `CM_Toolbar_Manager::ajax_reset_toolbar_settings()` (AJAX handler for resetting toolbar settings)

```php
// Hooked in menu-manager/class-toolbar-manager.php
add_action( 'wp_before_admin_bar_render', 'CM_Toolbar_Manager::capture_and_modify_toolbar' );
```
