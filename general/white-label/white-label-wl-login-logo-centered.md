...
> Center the login page logo in Classic Monks. Provides a balanced, professional login page layout.
...
The default WordPress login logo is left-aligned. Centering it creates a more balanced and professional layout.
...
This feature registers 1 WordPress hook in `custom-login-logo.php`:

**Actions:**

- `login_enqueue_scripts` calls `cm_custom_login_logo()` (Injects CSS for centered login logo (priority 10))
