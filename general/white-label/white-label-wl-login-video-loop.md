...
> Loop the login page video in Classic Monks. The background video plays continuously instead of stopping after one cycle.
...
A looping background video creates a dynamic, professional login page that doesn't go blank after one cycle.
...
- `login_enqueue_scripts` calls `cm_custom_login_video()` (Adds video background with loop attribute (priority 10))

```php
// Hooked in custom-login-page.php
add_action( 'login_enqueue_scripts', 'cm_custom_login_video' );
```
