...
> Mute the login page video in Classic Monks. Prevents the background video from playing audio.
...
Background videos on the login page can be distracting with audio. Muting ensures a silent, professional experience.
...
- `login_enqueue_scripts` calls `cm_custom_login_video()` (Adds video background with muted attribute (priority 10))
