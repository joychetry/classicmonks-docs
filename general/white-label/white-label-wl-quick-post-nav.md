...
> Add quick navigation between posts in the WordPress admin in Classic Monks. Navigate between posts without returning to the list.
...
The default WordPress flow requires going back to the post list to edit another post. Quick Post Nav creates direct navigation between posts.
...
This feature registers 4 WordPress hooks in `quick-post-nav/quick-post-nav.php`:

**Actions:**

- `admin_bar_menu` calls `CM_Quick_Post_Nav_Renderer::admin_bar_items()` (Adds Quick Post Nav items to admin bar)
- `wp_footer` calls `CM_Quick_Post_Nav_Assets::svg()` (Outputs SVG icons on frontend)
- `admin_footer` calls `CM_Quick_Post_Nav_Assets::svg()` (Outputs SVG icons in admin)
- `wp_enqueue_scripts` calls `CM_Quick_Post_Nav_Assets::enqueue_css()` (Enqueues QPN CSS on frontend)

```php
// Hooked in quick-post-nav/quick-post-nav.php
add_action( 'admin_bar_menu', 'CM_Quick_Post_Nav_Renderer::admin_bar_items' );
```
