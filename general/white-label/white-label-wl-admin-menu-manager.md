...
> Customize the WordPress admin menu in Classic Monks. Reorder, rename, hide, or reorganize menu items.
...
The default WordPress admin menu may not match your workflow. Customizing it creates a more efficient admin experience.
...
This feature registers 5 WordPress hooks in `menu-manager/class-menu-manager.php`:

**Actions:**

- `admin_menu` calls `CM_Menu_Manager::apply_menu_customizations()` (Renames, hides, and reorders admin menu items (priority 9999))
- `wp_ajax_cm_save_admin_menu` calls `CM_Menu_Manager::ajax_save_menu_settings()` (AJAX handler for saving menu settings)
- `wp_ajax_cm_reset_admin_menu` calls `CM_Menu_Manager::ajax_reset_menu_settings()` (AJAX handler for resetting menu settings)

**Filters:**

- `custom_menu_order` calls `apply_filters()` (Customizable filter)
- `menu_order` calls `apply_filters()` (Customizable filter)

```php
// Hooked in menu-manager/class-menu-manager.php
add_action( 'admin_menu', 'CM_Menu_Manager::apply_menu_customizations' );
```
