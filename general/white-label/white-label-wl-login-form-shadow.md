...
> Add a shadow effect to the login form in Classic Monks. Creates depth and visual hierarchy on the login page.
...
A shadow effect on the login form makes it stand out from the background and creates a more polished appearance.
...
- `login_enqueue_scripts` calls `cm_custom_login_page_style()` (Injects CSS for login form shadow (priority 10))
