...
Since Classic Monks 2.3.0, the mechanical part is automatic. Go to Classic Monks, Options, Migration and click Migrate on Perfmatters: the assistant scans the active Perfmatters setup and carries the covered settings across with per-category checkboxes and an instant summary. The full walkthrough, including what never transfers, is [How to Migrate from ASE or Perfmatters in WordPress](../docs/options/migrate-from-ase-perfmatters.md).

Perfmatters is still not a one-to-one replacement in either direction, so treat the remainder as a partial migration, done on staging first:
...
### Is there a one-click migration from Perfmatters?

Yes, for the settings Classic Monks covers. The Migration assistant (Classic Monks, Options, Migration) moves the quick toggles, lazy loading, CDN, analytics, login URL, and WooCommerce controls with per-category checkboxes. It never moves header, body, and footer code, the JS and CSS optimization pipeline, Script Manager rules, or database cleanup: those have no Classic Monks equivalent, so plan them manually or keep Perfmatters for them.
